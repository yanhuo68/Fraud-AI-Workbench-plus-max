...
- These scores come from FAISS; lower usually means closer/more similar (depending on the index/metric).
- Use the relative differences: a small gap between top scores implies similar relevance; a big gap implies a clear best match.
- When scores are close, review multiple chunks; when one score is much lower, favor that chunk’s source.
"""
        )
