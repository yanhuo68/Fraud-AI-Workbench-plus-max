...
def render_agents_ui():
    ...
    with col_llm:
        selected_llm = st.selectbox(
            ...
            key="agents_llm_select"
        )
    with col_llm_ref:
        st.write("") # padding
        if st.button("🔄 Scan", key="btn_scan_agents_llm", help="Scan for local LLMs (Ollama/LM Studio)"):
    ...
    if run["exception"]:
        ...
        with st.expander("🔴 Error Inspector"):
            from src.agents.error_inspector import render_error_inspector
...
from src.agents.error_inspector import capture_exception, annotate_trace_with_error
